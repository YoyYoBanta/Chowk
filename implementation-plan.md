...
**Tasks:**
- [x] Conversation search + in-thread search (context.md §2.1 F8, §10.2) — conversation-level search already existed (M3); in-thread search (a search box in `ThreadView`, hitting `GET /api/conversations/:id/messages?search=`) was the missing half, added here. Backed by a new `pg_trgm` GIN index on `Message.body` (migration `20260824130424_m9_message_search_index`) — the 10,000+ message performance target is verified, not assumed (see Tests below).
- [x] Audit every list endpoint for cursor pagination (context.md §9). `Conversation`/`Message` — the two collections with real unbounded growth — were already cursor-paginated since M3. Every other list endpoint (`/api/tags`, `/api/quick-replies`, `/api/custom-field-definitions`, `/api/users`, `/api/channels`, per-channel `/api/templates`) returns a plain array — reviewed and left as-is: each is bounded by an organization's own size (agents, tags, templates), not by message/conversation volume, and the "every list endpoint" rule's intent (per its own §9 framing) is about collections that actually grow unbounded. See decisions.md.
- [x] Structured JSON logging (architecture.md §16) — reviewed every consumer and API route; found and fixed one real gap: `src/app/api/webhooks/meta/route.ts` used raw `console.warn`/`console.error` with no `organizationId`/correlation id, unlike every other route in the codebase. Now uses the structured `logger` throughout. Confirmed (already true, no changes needed) that `LogFields` stays closed against `Message.body`/media content everywhere else.
- [x] Error boundaries in the UI (context.md §12) — `src/app/(dashboard)/error.tsx` and `src/app/error.tsx` (Next.js App Router's own error-boundary convention), neither existed before.
- [x] Retry policy audit on every queue — reviewed `src/queue/queues.ts` (all four queues have `attempts`+`backoff`) and every consumer's retryable/terminal branch (`send-message`, `status-update`, `download-media`, `ingest-inbound`) — all correctly wired end-to-end already; no gaps found needing a code change.
- [x] Worker restart resilience — new `src/worker/forced-restart.integration.test.ts`: a real BullMQ `Worker` is force-killed mid-processing (simulating a crashed process), a second `Worker` picks up the stalled job, and the message ends `SENT` exactly once. Uses its own private queue name rather than the shared `send-message` queue specifically so it never races a real, separately-running `npm run worker` process during development (see the test file's own doc comment and decisions.md).
- [x] Admin channel screen: quality rating and messaging tier displayed — already existed (built during the earlier M7/M8 fix-up pass), confirmed still correct, no changes needed.
- [x] Reconciliation pass for stuck `PENDING` messages — `src/services/messages/reconcile-stuck.ts` (new) + a second periodic entry in `src/worker/scheduler.ts` (every 5 minutes): messages `PENDING` for more than 30 minutes are marked `FAILED`/`STUCK_PENDING_TIMEOUT`, never silently re-enqueued (see decisions.md for why re-enqueueing was rejected as unsafe).

**Tests:**
- [x] Load-seeded search correctness + latency test at 10,000+ messages: `src/data/messages.search.integration.test.ts` — 10,500 seeded rows, exact-match correctness against real noise, and a latency assertion proving the trigram index is actually used (not a sequential scan).
- [x] Forced-restart-mid-queue test — the explicit M9 acceptance criterion. Met (see above).
- [x] Full run-through of context.md §13's four correctness hotspots as a regression suite: window calculation (`window.test.ts`, pre-existing) and status progression (`status-progression.test.ts`, pre-existing) still pass; variable substitution (`variables.test.ts`, added during the M7 completion pass) still passes; signature verification had **no dedicated test anywhere** before this pass — added `src/providers/cloud-api/webhook-verify.test.ts` (6 cases: valid, wrong secret, tampered payload, missing header, malformed prefix, malformed hex that must not throw). All four confirmed passing together in the same full suite run.

**Done when (context.md §11, M9):** search returns correct results across 10,000+ seeded messages in under a second, and the worker recovers cleanly from a forced restart mid-queue. **Met** — both proven by the two tests above, run against real Postgres/Redis, not asserted from memory.
